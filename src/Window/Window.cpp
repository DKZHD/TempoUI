...
Window::Window(int width, int height, const char* title) : window(glfwCreateWindow(width, height, title, nullptr, nullptr))
{
    ...
    ui_renderer = std::make_unique<UIRenderer>(FileReader::read_File("resources/themes/theme.json").c_str());
    ui_renderer->init(static_cast<float>(width), static_cast<float>(height));

    UI::GLFW::init_input(window);
    ...
    glfwSetFramebufferSizeCallback(*this, [](GLFWwindow* window, int x, int y)
    {
        if (Window* self = static_cast<Window*>(glfwGetWindowUserPointer(window)))
        {
            ...
            self->ui_renderer->get_element<Canvas>("Screen")->scale_updated(new_size / original_size);
        }
        glViewport(0, 0, x, y);
    });
    ...
    {
        test = 0.f;                                                              // NOLINT
        ui_renderer->get_element<ProgressBar>("PB")->bind_variable(test, 100.f); // NOLINT
        ui_renderer->get_element<GraphView>("View")->bind_variable(&test);
    });
...
void Window::process_input()
{
    ...
    {
        ThemeManager::get().load_theme(FileReader::read_File("../theme.json"));
    ...
    if (glfwGetKey(window, GLFW_KEY_F5) == GLFW_PRESS)
    {
        ...
        glm::vec2 new_size = { x, y };
        glm::vec2 original_size = { 1920, 1080 };
        ui_renderer->get_element<Canvas>("Screen")->scale_updated(new_size / original_size);
    }
}
